...
import type { AotComponent, AotType, SourceLocation, LiteralValue } from "./aot-ir.ts";
...
export function addAotContractBinding(component: AotComponent, ctx: ExpressionContext, name: string, sourceName: string, raw: ts.Type, at: SourceLocation, sourceNode?: ts.Node): void {
  ...
  const modelValue = [...ctx.modelModule?.signals ?? [], ...ctx.modelModule?.memos ?? []].find(value => value.name === sourceName);
  const modelFunction = ctx.modelModule?.functions.find(fn => fn.name === sourceName);
  const modelConstant = ctx.modelModule?.constants?.find(value => value.name === sourceName);
  if (raw.getProperty("__pocketNodeRef")) {
    (component.refs ??= []).push({ name, sourceName });
    return;
  }
  ...
  if (signatures.length) {
    ...
    const parameters = signature.getParameters().map(parameter => {
      ...
      return { name: parameter.name, type: modelFunction?.params.find(p => p.name === parameter.name)?.type ?? mapper.map(environment.checker.getTypeOfSymbolAtLocation(parameter, declaration), environment.locationOf(declaration), `${typeName(name)}${typeName(parameter.name)}`) };
    });
    const returns:AotType = modelFunction?.async ? {kind:"void"} : modelFunction?.returns ?? mapper.map(signature.getReturnType(), at, `${typeName(name)}Result`, true);
    ...
    const fn = { name, sourceName, parameters, returns, binding: false, handler: false, ...(optional ? { optional: true } : {}), ...(modelFunction?.async ? {async:true as const}: {}) };
    ...
    const typeIR = modelValue?.type ?? modelConstant?.value.type ?? (array ? { kind: "array" as const, element: mapper.map(elements![0]!, at, typeName(name)), length: array.length } : mapper.map(type, at, typeName(name)));
    const constant = modelConstant?.value.kind === "literal" ? modelConstant.value.value : type === raw ? array ?? mapper.literal(type) : undefined;
    const rawNumber = typeof constant === "number" ? modelConstant?.value.kind === "literal" ? modelConstant.value.rawNumber : constantNumericSpelling(environment.checker, sourceNode) : undefined;
    ...
    else component.values.push({ name, sourceName, type: typeIR, writable: false, ...(ctx.modelModule?.memos.some(memo => memo.name === sourceName) ? { memo: true } : {}) });
