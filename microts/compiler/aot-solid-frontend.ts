    ...
    for (const statement of script.statements) {
      if (ts.isImportDeclaration(statement)) {
        ...
        for (const node of clause.namedBindings.elements) {
          ...
          if (module === STD && !(sourceName in MICROTS_BUILTINS)) fail(at(node), `Unknown std built-in ${sourceName}`);
      ...
      function content(node: ts.Expression | Jsx, ctx: ExpressionContext, providerAllowed = false): AotNode[] {
        ...
        if (imp?.module === COMPONENTS && (kind === "ActionHandler" || kind === "AxisHandler" || kind === "MotionHandler")) {
          ...
          if (kind === "ActionHandler") {
            ...
            input = { kind: "axis", name: axis.value, axis: MICROTS_RELATIVE_AXES[axis.value as keyof typeof MICROTS_RELATIVE_AXES] };
          }
        ...
        for (const [name, attribute] of attributes) {
          if (name === "ref") {
            ...
            for (const property of object.properties) {
              ...
              const name = property.name.text, spec = MICROTS_STYLE_PROPS[name as keyof typeof MICROTS_STYLE_PROPS]; if (!spec) fail(loc(property), `Unknown numeric style property ${name}`);
