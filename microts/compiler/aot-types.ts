...
import { MICROTS_NUMERIC_TYPES, MICROTS_UNIT_TYPES } from "../../contracts/spec/microts.ts";
...
const NUMERIC = new Set<string>(MICROTS_NUMERIC_TYPES);
...
export function createTypeEnvironment(files: Map<string, string>, entry = files.keys().next().value as string): TypeEnvironment {
  ...
  const options: ts.CompilerOptions = {
    ...
    baseUrl: configured.baseUrl, paths: {
      ...
      "@pocketjs/framework/solid/std": [resolve(base, "framework/src/std-microts.ts")],
      ...
      "@pocketjs/framework/vue-vapor/std": [resolve(base, "framework/src/std-microts.ts")],
    },
  };
...
export class TypeMapper {
  ...
  private readonly units = new Map<keyof typeof MICROTS_UNIT_TYPES, AotType>();
  ...
  map(raw: ts.Type, loc: SourceLocation, hint: string, allowVoid = false): AotType {
    ...
    if (type.isIntersection()) {
      ...
      if (typeof label === "string" && label in MICROTS_UNIT_TYPES) {
        const unit = label as keyof typeof MICROTS_UNIT_TYPES;
        if (base.kind !== "number" || base.name !== MICROTS_UNIT_TYPES[unit].base) fail(loc, `${unit} requires the ${MICROTS_UNIT_TYPES[unit].base} representation`);
  ...
  unit(unit: keyof typeof MICROTS_UNIT_TYPES): AotType {
    ...
    this.declarations.push({ kind: "newtype", name, base: { kind: "number", name: MICROTS_UNIT_TYPES[unit].base }, unit });
    return value;
  }
