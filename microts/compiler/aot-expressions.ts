...
import { MICROTS_BUILTINS, parseMicroTsColor } from "../../contracts/spec/microts.ts";
...
export function expression(source: string, loc: SourceLocation, context: ExpressionContext, expected?: AotType): AotExpr {
  ...
  function build(node: ts.Expression, wanted?: AotType, ctx = context): AotExpr {
    ...
    if (ts.isCallExpression(node)) {
      ...
      const count = MICROTS_BUILTINS[builtin as keyof typeof MICROTS_BUILTINS].parameters.length;
...
function literal(value: string | number | boolean, expected: AotType | undefined, loc: SourceLocation, ctx: ExpressionContext, rawNumber?: string): AotExpr {
  ...
  if (actual) {
    ...
    if (typeof value === "string" && (actual.kind === "style" || actual.kind === "string")) type = actual;
      ...
      try { parseMicroTsColor(value); } catch { fail(loc, "Color literals use #rgb, #rgba, #rrggbb, or #rrggbbaa"); }
      type = actual;
