...
export function modelConfiguration(file: string): { mode: "rust" | "compiled"; recursionLimit: number; directory?: string; framework?: string; entry?: string } {
  for (let directory = dirname(resolve(file)); ; directory = dirname(directory)) {
    ...
    if (existsSync(path)) {
      ...
      return { mode, recursionLimit, directory, framework: app.framework, entry: app.entry };
    }
