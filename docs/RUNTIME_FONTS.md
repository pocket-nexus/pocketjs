...
**Worker cache budgets include lookup and eviction indexes.** Bitmap lookup uses the stable numeric glyph ID; shaping and layout lookup share immutable string keys with their entries. Ordered indexes replace scans, and pin counts maintain pinned-byte totals. Each nonempty cache charges sparse-root storage plus metadata per entry. The metadata allowance reduces the number of glyphs admitted by a fixed byte budget; it does not increase that budget. Allocation failure and cache refusal never select a UI-thread fallback.
