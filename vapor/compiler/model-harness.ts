...
export async function executeModelJavaScript(fixture: ModelHarnessCase, vue = false, transform?: (source: string) => string): Promise<ModelObservation[]> {
  ...
  const source = generateModelJavaScript(fixture.program, root, {
    ...
    stdImport: resolve("framework/src/std-vue-vapor.ts"), tasksImport: resolve("framework/src/model-tasks.ts"),
  });
...
const rustHarness = `
use pocket_vapor::model::{ModelValue, ModelTrace, Value, take_trace, reset_trace};
...
fn task(t: pocket_vapor::model::TaskId) -> Json { json!({"region":t.region,"fn":t.function,"generation":t.call}) }
fn request(r: pocket_vapor::model::RequestId) -> Json { json!({"task":task(r.task),"generation":r.wait,"member":r.member}) }
fn counts() -> Json { let c=pocket_vapor::model::take_counts(); json!({"scheduledMemos":c.scheduled_memos,"demandMemos":c.demand_memos,"effects":c.effects,"taskSegments":c.task_segments,"loopIterations":c.loop_iterations}) }
...
export async function executeModelRust(fixtures: readonly ModelHarnessCase[], options: { release?: boolean } = {}): Promise<Map<string, ModelObservation[]>> {
  ...
  writeFileSync(resolve(directory, "Cargo.toml"), `[package]\nname="model-aot-differential-${identity}"\nversion="0.0.0"\nedition="2021"\n[workspace]\n[dependencies]\npocket_vapor={path=${JSON.stringify(resolve("engine/crates/pocket-vapor"))},features=["std","model-trace"]}\nserde_json="1"\n`);
  ...
  fixtures.forEach((fixture, index) => {
    ...
    const body = fixture.tape.map((input, frame) => {
      ...
      const deliveries = (input.deliveries ?? []).map(({ request: r, value }) => `pocket_vapor::model::Delivery {request:pocket_vapor::model::RequestId{task:pocket_vapor::model::TaskId{region:${r.task.region},function:${r.task.fn},call:${r.task.generation}},wait:${r.generation},member:${r.member}},result:pocket_vapor::model::Completion::Value(decode(serde_json::from_str(${JSON.stringify(JSON.stringify(value))}).unwrap()))}`).join(",");
      return `m.resume(&pocket_vapor::Ready {frame:${frame + 1},now_ms:${Number(input.clock ?? 0).toString()}f64,services:vec![${services}],deliveries:vec![${deliveries}],..Default::default()}, &mut commands);\n${dispatch}\nif m.model_changed() || ${!!input.dispatch?.length || !!input.invalidate} { m.react(false,&mut commands); m.settle(); }\nassert!(!m.model_changed(),"changed flag survived frame");\nframes.push(json!({"frame":${frame + 1},"state":encode(m.model_state()),"trace":take_trace().into_iter().map(event).collect::<Vec<_>>(),"counts":counts() }));`;
    }).join("\n");
    invocations.push(`for _replay in 0..2 { use case_${index}::*; reset_trace(); let mut m=${root.name}Model::default(); let mut commands=Vec::new(); m.react(true,&mut commands);m.settle();take_trace();pocket_vapor::model::reset_counts();let mut frames=Vec::<Json>::new(); ${body}\nprintln!("{}",json!({"name":${JSON.stringify(fixture.name)},"frames":frames})); }`);
  });
