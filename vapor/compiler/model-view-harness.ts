...
import { buildVueAot } from "./aot-build.ts";
...
export async function executeModelView(directory: string) {
  ...
  const result=await buildVueAot(fixture,{strict:true,outDir:resolve(run,"src/gen"),format:false});
...
mod gen; use gen::*; use pocket_vapor::{Host,Ui,Input,NodeId,Cmd}; use serde_json::{json,Value}; use std::cell::RefCell;
...
impl<const B:u32> pocket_vapor::HasButton<B> for Native{}
fn text(ui:&Ui,id:i32)->Vec<String>{if let Some(style)=ui.core().resolved_style(id){if style.display==pocket_vapor::spec::Display::None as u8{return vec![];}}if ui.core().node_type(id)==Some(1){return vec![ui.core().node_text(id).unwrap().into()];}ui.core().node_children(id).iter().flat_map(|id|text(ui,*id)).collect()}
