...
const work = (kind: string): RustStatement => re(rc(rp("pocket_vapor", "model", "count_work"), rp("pocket_vapor", "model", "ModelWork", kind)));
...
class ModelRust {
  ...
  coerce(e: ModelExpr, type: AotType, name = "value", capacity?: number): RustExpr {
    ...
    if (type.kind === "string" && actual !== capacity) return capacity === undefined
      ...
      : rc({ kind: "path", path: ["pocket_vapor", "model", "bounded_string"], typeArgs: [{ kind: "const", value: capacity }] }, ref(value), rl(name));
    if (type.kind === "array" && type.length === undefined && actual !== capacity) return capacity === undefined
      ...
      : rc({ kind: "path", path: ["pocket_vapor", "model", "bounded_array"], typeArgs: [this.type(type.element), { kind: "const", value: capacity }] }, value, rl(name));
    return value;
  }
  ...
  expr(e: ModelExpr, capacity?: number, capacityName = "value"): RustExpr {
    ...
    switch (e.kind) {
      case "literal": {
        if (e.type.kind === "named") { const d = this.declarations.get(e.type.name); if (d?.kind === "enum") return rp(d.name, rustVariant(String(e.value))); if (d?.kind === "newtype") return rc(rp(d.name), d.unit === "Color" ? rl(parseVaporColor(String(e.value)), "u32") : this.expr({ ...e, type: d.base })); }
        if (typeof e.value === "string") return capacity === undefined ? rc(rp("String", "from"), rl(e.value)) : rc({ kind: "path", path: ["pocket_vapor", "model", "bounded_string"], typeArgs: [{ kind: "const", value: capacity }] }, rl(e.value), rl(capacityName));
      ...
      case "binary": {
        ...
        if (e.operator === "+" && type.kind === "string") {
          ...
          result = capacity === undefined ? rc(rp("pocket_vapor", "model", "concat"), ref(textLeft), ref(textRight)) : rc({ kind: "path", path: ["pocket_vapor", "model", "bounded_concat"], typeArgs: [{ kind: "const", value: capacity }] }, ref(textLeft), ref(textRight), rl(capacityName));
      ...
      case "template": {
        ...
        for (const part of e.parts) statements.push(re(rc(rp("pocket_vapor", "model", capacity === undefined ? "append_display" : "append_bounded_display"), ref(rp(name), true), ref(typeof part === "string" ? rl(part) : this.displayValue(this.expr(part), part.type)), ...(capacity === undefined ? [] : [rl(capacityName)]))));
  ...
  builtin(e: Extract<ModelExpr, { kind: "builtin" }>, capacity?: number, capacityName = "value"): RustExpr {
    ...
    if (e.name === "String") {
      ...
      if (capacity === undefined) return rc(rp("pocket_vapor", "model", "to_string"), ref(value));
      const name = `text_${this.serial++}`;
      return block([let_(name, rc({ kind: "qualifiedPath", type: this.type({ kind: "string" }, false, capacity), member: "new" }), true), re(rc(rp("pocket_vapor", "model", "append_bounded_display"), ref(rp(name), true), ref(value), rl(capacityName)))], rp(name));
    }
    ...
    if (["map", "filter", "find", "some"].includes(e.name)) {
      ...
      if (e.name !== "find" && e.type.kind === "array" && capacity !== undefined) return block(statements, rc({ kind: "path", path: ["pocket_vapor", "model", "bounded_array"], typeArgs: [this.type(e.type.element), { kind: "const", value: capacity }] }, iter, rl(capacityName)));
      return block(statements, e.name === "find" ? iter : { kind: "method", object: iter, method: "collect", args: [], typeArgs: [this.type(e.type)] });
    }
    return this.wrapScalar(rc(rp("pocket_vapor", "builtins", e.name), ...args.map((arg, index) => this.unwrapScalar(arg, e.args[index]!.type))), e.type);
  }
      ...
      case "external": {
        if (s.op === "log") {
          ...
          return [...args.map(arg => let_(arg.name, arg.value)), condition({ kind: "macro", name: ["cfg"], args: [rp("debug_assertions")] }, [
            ...
            let_(name, rc(rp("String", "new")), true), ...args.flatMap((arg, index) => [...(index ? [re(rm(rp(name), "push_str", rl(" ")))] : []), re(rc(rp("pocket_vapor", "model", "append_display"), ref(rp(name), true), ref(this.displayValue(rp(arg.name), arg.type))))]), re(rm(field("commands"), "push", rc(rp("Cmd", "Log"), rp(name)))),
          ])];
        }
  ...
  decodeCompletion(type: AotType, completion: RustExpr): RustExpr {
    const decodeValue = (type: AotType, value: RustExpr): RustExpr => type.kind === "tuple"
      ? { kind: "tuple", elements: type.elements.map((element, index) => decodeValue(element, rc(rp("pocket_vapor", "model", "value_element"), value, rl(index)))) }
      : rm(rc({ kind: "qualifiedPath", type: this.type(type), member: "from_model_value" }, value), "expect", rl("malformed task tuple completion"));
    return type.kind === "tuple" ? decodeValue(type, rc(rp("pocket_vapor", "model", "completion_value"), completion)) : rc({ kind: "path", path: ["pocket_vapor", "model", "decode"], typeArgs: [this.type(type)] }, completion);
  }
  ...
  codecs() {
    ...
    for (const d of this.program.types) {
      ...
      if (d.kind === "struct") {
        ...
        encode = d.unit === "Color" ? rc(rp("Value", "String"), rc(rp("pocket_vapor", "model", "to_string"), self)) : rm(rf(self, 0), "model_value");
        decode = rm(d.unit === "Color" ? rc(rp("pocket_vapor", "model", "decode_color"), rp("value")) : rc({ kind: "qualifiedPath", type: this.type(d.base), member: "from_model_value" }, rp("value")), "map", rp(d.name));
      }
    ...
    for (const d of this.program.types) {
      ...
      if (d.kind === "newtype" || d.kind === "enum") {
        ...
        const display: RustExpr = d.kind === "enum" ? { kind: "match", value: self, arms: d.variants.map(name => ({ pattern: { kind: "variant", path: [d.name, rustVariant(name)] }, body: rm(rp("formatter"), "write_str", rl(name)) })) }
          : d.unit === "Color" ? rc(rp("pocket_vapor", "format_color"), rf(self, 0), rp("formatter")) : rc(rp("pocket_vapor", "VaporDisplay", "fmt_vapor"), ref(rf(self, 0)), rp("formatter"));
        this.items.push({ kind: "impl", type: rt(d.name), trait: rt("pocket_vapor::VaporDisplay"), methods: [fn("fmt_vapor", [receiver(), formatter], rb([], display), rt("core::fmt::Result"))] });
      }
    }
  }
  ...
  animation(op: "animate" | "jump", args: ModelExpr[], request: RustExpr = rp("None"), values = args.map(arg => this.expr(arg))): RustExpr {
    ...
    const propValue: RustExpr = prop ? rl(PROP[prop], "u8") : { kind: "match", value: rc(rp("pocket_vapor", "model", "value_string"), ref(rm(values[1]!, "model_value"))), arms: [...ANIMATABLE.map(name => ({ pattern: { kind: "variant" as const, path: ["Some"], tuple: [{ kind: "literal" as const, value: name }] }, body: rl(PROP[name], "u8") })), { pattern: { kind: "wildcard" }, body: { kind: "macro", name: ["panic"], args: [rl("model animation property is not animatable")] } }] };
    ...
    const numericTarget = target.type.kind === "string" ? rc(rp("pocket_vapor", "model", "animation_color"), ref(values[2]!)) : cast(values[2]!, rt("f64"));
