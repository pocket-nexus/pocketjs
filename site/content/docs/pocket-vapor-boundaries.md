...
Pocket Vapor adds a Rust compilation path for supported views and models.
...
| View operations | Framework renderer emits `HostOps` through `ui.*`; it maintains a mirror tree | Generated Rust view calls `pocket_vapor::Ui` |
| Input | Host frame samples enter the framework input bridge | Host samples become `pocket_vapor::Input` |
...
schedule; the [model guide](/docs/pocket-vapor-model/) lists the Vue forms.
