...
impl FeatureToggleNode14 {
    ...
    fn dispatch_step<M: FeatureToggleViewModel>(
        ...
        if cursor.visit() && input.is_press(self.node) {
            let handler_arg0 = vm.press();
            events.push(FeatureToggleEvent::Toggle(handler_arg0));
            return true;
        };
        self.children
            .dispatch_step(input, props, vm, __pocket_inject_0, events, cursor)
...
impl ModelButtonNode20 {
    ...
    fn dispatch_step<M: ModelButtonViewModel>(
        ...
        let mut handled = false;
        if cursor.visit() && input.is_press(self.node) {
            let handler_arg0 = props.value.wrapping_add(1i32);
            events.push(ModelButtonEvent::Change(handler_arg0));
            return true;
        };
        self.children
            .dispatch_step(input, props, vm, events, cursor)
...
impl AppNode57 {
    ...
    fn dispatch_step<M: AppViewModel>(
        ...
        let mut handled = false;
        if cursor.visit() && input.is_press(self.node) {
            let __pocket_emit_32_0 = vm.count().wrapping_add(1i32);
            let handler_value = __pocket_emit_32_0;
...
struct AppFor103 {
    rows: KeyedList<String, AppBlock104>,
    parent: NodeId,
    handler_ends: Vec<usize>,
}
...
struct AppBlock34<StateFeatureToggle: FeatureToggleViewModel + Default + 'static> {
    ...
    child2: AppNode39,
    child3: AppNode49,
    child4: AppNode94<StateFeatureToggle>,
    child5: AppNode99,
}
