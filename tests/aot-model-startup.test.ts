...
createEffect(()=>{setCount(5);setVisible(false);});async function byFrame():Promise<void>{await frames(1);setFramed(1);}async function byTime():Promise<void>{await after(16);setTimed(1);}createEffect(()=>{byFrame();byTime();});export function press():void{setSeen(count());}`,
  ...
  const result=await buildVueAot(fixture,{outDir:resolve(run,"src/gen"),format:false});
...
`);
  ...
  await Bun.write(resolve(run,"src/main.rs"),`mod gen;use gen::*;use pocket_vapor::{Host,Ui,Input,Cmd};use std::cell::RefCell;
thread_local!{static COMMANDS:RefCell<Vec<String>>=const{RefCell::new(Vec::new())};}struct Native(Ui);impl Host for Native{fn ui(&self)->&Ui{&self.0}fn ui_mut(&mut self)->&mut Ui{&mut self.0}fn into_ui(self)->Ui{self.0}fn model_command(&mut self,c:Cmd){if let Cmd::Log(s)=&c{COMMANDS.with(|v|v.borrow_mut().push(s.clone()));}self.0.model_command(c);}}impl<const B:u32>pocket_vapor::HasButton<B> for Native{}
