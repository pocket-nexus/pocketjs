...
use pocket_solid_aot_lab::{LabApp,AppModel,AppViewModel};use pocket_vapor::{Ui,Input,NodeId};
...
for frame in 0..2048 {let input=match frame%8{0=>Input::press(increment),2|3=>Input::default().with_axis(0,7500),4|6=>Input::press(toggle),7=>Input::buttons(pocket_vapor::spec::btn::CROSS),_=>Input::default()};let started=Instant::now();std::hint::black_box(&mut app).frame(input);frames.push(started.elapsed().as_nanos()as u64);
