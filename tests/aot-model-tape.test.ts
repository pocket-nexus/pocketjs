...
test("motion tape states fire handlers at their quality with derived values and metadata", () => {
  const view = program({
  ...
  const turned = { timestamp: 2000, gravityDirection: { value: [-1, 0, 0] as const, quality: 4 as const }, rotationRate: { value: [0, 0, 90] as const, quality: 3 as const } };
  const flat = { timestamp: 3000, gravityDirection: { value: [0, 0, -1] as const, quality: 4 as const } };
  const frames = replayModelTape(view, [{ motion: { timestamp: 1000, gravityDirection: { value: [0, -1, 0], quality: 3 } } }, { motion: turned }, { buttons: 0 }, { motion: flat }]);
